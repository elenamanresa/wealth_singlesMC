...
library(reticulate)
conda_list()
use_condaenv("py3.6-tensorflow-cpu")
...
param_set = list(cfloor,delta,beta,nu,phi0,K0,thres)
...
system2("C:/wealth_singles - MC/ccode/wealth21_singles/test1/x64/Release/test1.exe", invisible = FALSE)
...
cfloor0 = 8.411833
nu0_grid = c(3,4,5,6)
phi0_grid = c(0.1,0.2,0.3,0.4)
k0_grid = c(5,10,20,30)

matrix_grid = matrix(0,64,3)
...
for(grid_num in 1:64){
  
  cfloor = cfloor0
  delta = 0;
  beta  = 0.97106
  nu = matrix_grid[grid_num,1]
  phi0 = matrix_grid[grid_num,2]
  K0 =  matrix_grid[grid_num,3]
  phi00 = phi0
  thres = 0
  ...
  loss_f_mat[grid_num]<-Loss(table_aux_loss)
  print(loss_f_mat[1:grid_num])
...
delta = 0;
beta  = 0.97106
nu = matrix_grid[grid_best[rep],1]
phi0 = matrix_grid[grid_best[rep],2]
K0 =  matrix_grid[grid_best[rep],3]
phi00 = phi0
thres = 0
      ...
      for(grad_ii in 1:num_param){
        if(optim_param[grad_ii]==1){
          if(grad_mat[cur,grad_ii]*grad_mat[cur_1,grad_ii]<0){
            step_g_mat[grad_ii] <- max(step_g_mat[grad_ii]*0.8,0.001*step_g_mat0[grad_ii])
            #step_g_mat[grad_ii] <- max(step_g_mat[grad_ii]*1,0.001*step_g_mat0[grad_ii])
          }else{
            step_g_mat[grad_ii] <- min(step_g_mat[grad_ii]*1.2,100*step_g_mat0[grad_ii])
...
uu = which.min(loss_f)
best_mat[rep,1] = loss_f[uu]
best_mat[rep,2] = loss_f[1]
best_mat[rep,3:8] = parmvec_mat[uu,1:6]
best_mat[rep,9] = kkk_old
...
save(best_mat,file=paste("res_lcfloor3_",grid_num,"_",rep,".Rdata",sep=""))
