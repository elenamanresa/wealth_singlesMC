...
proc(8) = getprofs();
    ...
    local data, datstr, datah, dataw, k, j, ib, mortshg, mortshb, mortswg, 
          ...
          ggbb, gg, bb, _g, _b, yprofsh, yprofsw, yprof, y_pi, copays,
          rorshk, wlthshk, onevec, label1;
   ...
   /* Pr(alive_t+1|..) = sqrt( logit(b_const[age] + b_badhealh[age] + ...) ) */
    ...
    label1 = "       age       hbad      hgood       wbad      wgood";
  ...
  /*-----------------------------Income Profiles-----------------------------*/
   ...
   /* y = exp(b_const[age] + b_badhealh[age] + ... )                         */
    ...
    if sumc(data[.,3]) /=0 ;
       "WARNING:  Non-zero health effects are being ignored ";
    endif;

    "Income Profiles at the ";; pctle;; " PI Percentile";
    "       age     males    females";;
    yprofsh  = datah[.,2];           /* Health effects have been set to zero */
    yprofsw  = dataw[.,2];
  ...
  /*---------------------------Copayment Rates (q)---------------------------*/
    datstr   = datapath$+"medpriceprof.out";
    loadm data[] = ^datstr;
    data     = reshape(data,rows(data)/4,4);

   /* Order: age, b_const, b_badhealth b_male                                */
   /*        there are separate values of b_* for each age                   */
   /* q = exp(b_const[age] + b_badhealh[age] + ... )                         */

    datah    = data[.,1]~(data[.,2]+data[.,4])~data[.,3];
    {k,j,ib} = getages(bornage,dieage,datah);
    datah    = fixobs(datah,bornage,dieage,k,j,0,-1e10);
    dataw    = data[.,1]~data[.,2]~data[.,3];
    dataw    = fixobs(dataw,bornage,dieage,k,j,0,-1e10);

    "Copayment Rates (all PI values)";
    label1;;
    copays   = (datah[.,2]+datah[.,3])~(datah[.,2]);
    copays   = copays~(dataw[.,2]+dataw[.,3])~(dataw[.,2]);
    copays   = exp(copays);
    ageseq~copays;?;  
...
retp(morts, healdats, yprof, mort_pi, heal_pi, y_pi, copays, rorshk); 
endp;
...
proc(4) = getmxtab(mxcoef,pctle,smplyrs);
    ...
    local b_const, b_badh, b_male, b_PI, b_PI2, mnmxs, mnmx_pi, stdmxs, 
 ...
 /* First for the means */
    ...
    b_badh  = mxcoef[5,1] + mxcoef[6,1]*ageseq;
    b_male  = mxcoef[7,1] + mxcoef[8,1]*ageseq;
    b_PI    = mxcoef[9,1] + mxcoef[10,1]*ageseq;
    b_PI2   = mxcoef[11,1]*ones(_TR,1);
    mnmxs   = (b_const[1:_TR]+b_badh[1:_TR])~b_const[1:_TR];
    mnmxs   = (b_const+b_badh+b_male)~(b_const+b_male)~mnmxs;
    mnmx_pi = b_PI[1:_TR]~b_PI2[1:_TR];
    mnmx_pi = b_PI~b_PI2~mnmx_pi;

 /* Then for the variances */

    b_const  = mxcoef[1,2] + mxcoef[2,2]*ageseq + mxcoef[3,2]*(ageseq^2/100)
               + mxcoef[4,2]*(ageseq^3/10000);
    b_badh   = mxcoef[5,2] + mxcoef[6,2]*ageseq;
    b_male   = mxcoef[7,2] + mxcoef[8,2]*ageseq;
    b_PI     = mxcoef[9,2] + mxcoef[10,2]*ageseq;
    b_PI2    = mxcoef[11,2]*ones(_TR,1);
    stdmxs   = (b_const[1:_TR]+b_badh[1:_TR])~b_const[1:_TR];
    stdmxs   = (b_const+b_badh+b_male)~(b_const+b_male)~stdmxs;
    varmx_pi = b_PI[1:_TR]~b_PI2[1:_TR];
    varmx_pi = b_PI~b_PI2~varmx_pi;
    ...
    if pctle /= -1;
        ...
        ?;" Analytical Mean Medex Utility Shifter at the ";; pctle;; " PI Percentile";
        title1;;
        ageseq[smplyrs]~mxmean[smplyrs,.];?;

        " Analytical Std. deviation Medex Utility Shifter at the ";; pctle;; " PI Percentile";
        title1;;
        ageseq[smplyrs]~mxstd[smplyrs,.];?;

        " 99.5-th Percentile Annual Medex Utility Shifter at the ";; pctle;; " PI Percentile";
        title1;;
...
proc(15) = getdata();
 ...
 @  obsdat   = (obsdat+wlthdat[.,27+ashft2]).>0; @
    obsdat   = 1-obsdat;
...
proc(0) = initdist(isimage);
    ...
    local ageindx, gotobs, gotobs2, num0, rn, num, agesim96, pisim96, mstat96,  
          ...
          hhdx, hwdx, hmissdx, toosmall, bumpup;
  ...
  /*-----------------a^ = a + u, u orthogonal to a, v------------------------*/
    ...
    elseif simtype==2;
        ...
        save path= ^shkpath asimx, incsim96x, mxsim96x, cohsimx, mstatx, 
                            ...
                            mstatdx, hhdx, hwdx, hmissdx, simwgtx,
                            HHIDx;
    endif;
...
proc(0) = initsim(morts, mort_pi, healdats, heal_pi);
    ...
    local hsimh96, hsimw96, mstat96, pisim96, agesim96, ageindx,
          hsimhx, hsimwx, mstatx, pisimx, agesimx, ii, ai2,  
...
proc(0) = initsim2(morts, mort_pi, healdats, heal_pi);
    ...
    local hsimh96, hsimw96, mstat96, pisim96, agesim96, msdat, hhdat, hwdat, 
          hmissdat, healsimh, healsimw, hsimhx, hsimwx, mstatx, pisimx, 
          agesimx, mstatdx, hhdx, hwdx, hmissdx, hsimpoh, hsimpow, mstatsim, 
          mstpo, agesim, ageindx, jj, ii, ii_L, ii_H, clndryr, hmiss, msmiss, 
          ai2, too_old, healshk,
  ...
  /*---Construct health status imputation probabilities using Baye's Rule----*/
    ...
    elseif simtype==2;
        ...
        hhdat   = hhdx;    hwdat   = hwdx;     hmissdat = hmissdx;
...
*/
            ...
            _pcolor = ones(3,1).*.plc0[2|4];       /* Common colors for Cohorts */
