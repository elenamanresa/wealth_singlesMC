...
train_model2 <- function(x_train, y_train, x_test, y_test, epoch_p,model_keras,file_name){
  ...
  history <- fit(
    ...
    callbacks        = list(callback_early_stopping(monitor = "loss",patience = 100,verbose = 0), callback_model_checkpoint(file_name,monitor='acc',save_best_only = TRUE,verbose = 0)),
    verbose          = 0
  )
  #end_time <-Sys.time()
  #print('Network Training time')
